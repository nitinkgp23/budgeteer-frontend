...
FinancialEntityCategoryView buildFinancialEntityFromTransactionData(
    TransactionData model,
    int transactionDataIndex,
    BuildContext context,
    ) {
  final amount = rupeeWithSignFormat(context).format(model.amount);
  final dateString = dateFormatAbbreviatedMonthDay(context)
      .format(model.date);
  ...
  return FinancialEntityCategoryView(
    ...
    indicatorColor: RallyColors.accountColor(transactionDataIndex),
    indicatorFraction: 1,
    amount: amount,
  );
}
...
FinancialEntityCategoryView buildFinancialEntityFromAccountData(
  ...
  final amount = rupeeWithSignFormat(context).format(model.primaryAmount);
...
FinancialEntityCategoryView buildFinancialEntityFromBudgetData(
  ...
  final amountUsed = rupeeWithSignFormat(context).format(model.amountUsed);
  final primaryAmount =
      rupeeWithSignFormat(context).format(model.primaryAmount);
  final amount = rupeeWithSignFormat(context)
...
List<FinancialEntityCategoryView> buildTransactionDataListViews(
    List<TransactionData> items,
    BuildContext context,
    ) {
  return List<FinancialEntityCategoryView>.generate(
    items.length,
        (i) => buildFinancialEntityFromTransactionData(items[i], i, context),
  );
}
...
class _EventAmount extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final textTheme = Theme.of(context).textTheme;
    return Text(
      rupeeWithSignFormat(context).format(amount),
      style: textTheme.bodyText1.copyWith(
        fontSize: 20,
        color: RallyColors.gray,
      ),
